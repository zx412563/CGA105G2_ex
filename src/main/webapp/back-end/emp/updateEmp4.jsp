...
<body>
						...
						<div class="collapse navbar-collapse" id="navbarSupportedContent">
							<ul class="navbar-nav ml-auto ">
								...
								<li class="nav-item"><a class="nav-link text-uppercase"
									data-toggle="none" href="./ContactUs.html"> 🚩前台查詢 </a></li>
								<li class="nav-item"><a class="nav-link text-uppercase"
									data-toggle="none" href="./Backstage-4.html"> ✔️審核 </a></li>
								<li class="nav-item"><a class="nav-link text-uppercase"
									data-toggle="none" href="./Waiting.html"> 📔訂單 </a></li>
								<li class="nav-item"><a
									class="nav-link text-uppercase active "
									data-toggle="none" href="<%=request.getContextPath()%>/back-end/emp/empIndex2.jsp"> 🗃️管理 </a></li>
								<li class="nav-item"><a class="nav-link text-uppercase"
									data-toggle="none" href="./Waiting.html"> 🚪Sing out </a></li>
							</ul>
						</div>
					</nav>
				</div>
	...
	<div class="container-fluid">
		<div class="row">
			...
			<main role="main"
				class="col-md-9 ml-sm-auto col-lg-10 px-md-4 shadow">
				<div
					class=" d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-15 mt-5 border-bottom">
					<h1 class="h2">🔆員資料修改</h1>

				</div>
				<div class=" shadow card-body  rounded mb-20 bg-secondary "
					id="div11" style="border-radius: 30px;">
					<form METHOD="post"
						ACTION="<%=request.getContextPath()%>/back-end/emp/test"
						name="form1">
						<div class="form-group ">
							<label for="exampleInputname">員工編號</label><br>
							<p><%=employee.getEmpId()%></p>
						</div>
						<div class="form-group">
							<label for="exampleInputPassword1">員工狀態</label><br>
							<td><select size="1" name="empstatus">
						<option value="<%=employee.getEmpStatus()%>"> <%=(employee.getEmpStatus() == 0) ? "在職" : "離職"%> </option>
						<option value="<%=(employee.getEmpStatus() == 0) ? "1" : "0"%>"><%=(employee.getEmpStatus() == 1) ? "在職" : "離職"%></option>
				</select></td>
						</div>
						<div class="form-group">
							<label for="exampleInputPassword1">員工帳號</label><br> <input
								type="TEXT" name="empacc" value=<%=employee.getEmpAcc()%> />
						</div>
						<div class="form-group">
							<label for="exampleInputPassword1">員工密碼</label><br> <input
								type="TEXT" name="emppwd" value=<%=employee.getEmpPwd()%> />
						</div>
						<div class="form-group">
							<label for="exampleInputPassword1">員工權限</label><br>
							<td><select size="1" name="empper">
									<option value="<%=employee.getEmpPer()%>"><%=(employee.getEmpPer() == 0) ? "員工" : "主管"%></option>
									<option value="<%=(employee.getEmpPer() == 0) ? "1" : "0"%>"><%=(employee.getEmpPer() == 1) ? "員工" : "主管"%></option>
							</select></td>
						</div>
						<div class="form-group">
							<label for="exampleInputPassword1">新增時間</label><br>
							<p><%=employee.getEmpTime()%></p>
						</div>

						<div class="form-group">
							<label for="exampleInputPassword1" >修改時間</label><br> <input
								 name="EmpRtime" id="aaa" value="acccca" disabled ></input>
						</div>
						...
						<button type="submit" class="btn btn-outline-dark mb-1 "
							...
							onclick="addEmpAlert()">Submit</button>
					</form>
				</div>
			</main>
		</div>
