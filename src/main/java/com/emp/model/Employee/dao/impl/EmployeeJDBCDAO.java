...
public class EmployeeJDBCDAO implements EmployeeDAO_interface {
	...
	public void update(Employee EmployeeVO) {
		...
		try (Connection con = DriverManager.getConnection(Common.URL, USER, PASSWORD);
			...
			pstmt.setTimestamp(5, EmployeeVO.getEmpTime());
			pstmt.setTimestamp(6, EmployeeVO.getEmpRtime());
	...
	public Employee findByEMP_ID(Integer EMP_ID) {
		...
		try (Connection con = DriverManager.getConnection(Common.URL, USER, PASSWORD);
			...
			while (rs.next()) {
				...
				employeeVO.setEmpTime(rs.getTimestamp("EMP_TIME"));
				employeeVO.setEmpRtime(rs.getTimestamp("EMP_RTIME"));
