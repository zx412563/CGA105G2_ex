...
public class CartJDBCDAO implements CartDAO_interface {
		...
		public void insert(Cart cart) {
			final String sql =
					"INSERT INTO `cart` (mem_id,goods_id,cart_num) VALUES (?, ?, ?)";
		...
		public void update(Cart cart) {
			final String sql =
					"UPDATE `cart` set mem_id=?,goods_id=?,cart_num=? where mem_id= ? AND goods_id = ?";
			try (Connection con = DriverManager.getConnection(Common.URL, USER, PASSWORD);
				 PreparedStatement pstmt = con.prepareStatement(sql, ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY)) {
				pstmt.setInt(1, cart.getMemId());
				pstmt.setInt(2, cart.getGoodsId());
		...
		public void delete(Integer cartno) {
			final String sql =
					"DELETE FROM `cart` where mem_id= ? AND goods_id = ?";
			try (Connection con = DriverManager.getConnection(Common.URL, USER, PASSWORD);
				 PreparedStatement pstmt = con.prepareStatement(sql, ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY)) {
		...
		public Cart getById(Integer cartno) {
			final String sql =
					"SELECT mem_id,goods_id,cart_num FROM `cart` where mem_id= ? AND goods_id = ?";
			Cart cart = null;
			try (Connection con = DriverManager.getConnection(Common.URL, USER, PASSWORD);
				 PreparedStatement pstmt = con.prepareStatement(sql, ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY)) {
				pstmt.setInt(1, cartno);
		...
		public List<Cart> getAll() {
			final String sql =
					"SELECT mem_id,goods_id,cart_num FROM `cart`";
